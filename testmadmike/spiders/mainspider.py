...
class logger():
    def info(self, msg='Information'):
        line = 'INFORMATION: {} - {}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), msg)
        print(colors.yellow(line))
        self.write('log', line)

    def success(self, msg='Success'):
        line = 'SUCCESS: {} - {}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), msg)
        print(colors.green(line))
        self.write('log', line)

    def error(self, msg='Error'):
        line = 'ERROR: {} - {}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), msg)
